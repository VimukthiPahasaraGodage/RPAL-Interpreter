...
public class Parser{
  ...
  private void Proc_E(){
    if(isCurrentToken(Scanner.TOKEN_TYPE_RESERVED, "let")){ //E -> 'let' D 'in' E => 'let'
      ...
      Read_Next();
      Proc_E(); //this is extra readNT in proc_E()
  ...
  private void procBP(){
    ...
    if(isCurrentToken(Scanner.TOKEN_TYPE_RESERVED,"gr")||isCurrentToken(Scanner.TOKEN_TYPE_OPERATOR,">")){ //Bp -> A('gr' | '>' ) A => 'gr'
      Read_Next();
      procA(); //THIS IS to extra readNT in procA()
  ...
  private void procA(){
    if(isCurrentToken(Scanner.TOKEN_TYPE_OPERATOR, "+")){ //A -> '+' At
      Read_Next();
      Proc_AT(); //This is to extra readNT in procAT()
    }
  ...
  private void Proc_AT(){
    ...
    while(isCurrentToken(Scanner.TOKEN_TYPE_OPERATOR, "*")||isCurrentToken(Scanner.TOKEN_TYPE_OPERATOR, "/")){
      if(Cur_Token.getTokenValue().equals("*"))
        ...
        mult = false;
      Read_Next();
      procAF(); //This is to extra readNT in procAF()
      if(mult) //At -> At '*' Af => '*'
  ...
  private void procAP(){
    ...
    while(isCurrentToken(Scanner.TOKEN_TYPE_OPERATOR, "@")){ //Ap -> Ap '@' '<IDENTIFIER>' R => '@'
      ...
      Read_Next();
      procR(); //This is to extra readNT in procR()
  ...
  private void procRN(){
    if(Is_Cur_Token_Type(Scanner.TOKEN_TYPE_IDENTIFIER)|| //R -> '<IDENTIFIER>'
      ...
      Read_Next();
      Proc_E(); //This is to extra readNT in procE()
  ...
  private void procDA(){
    ...
    while(isCurrentToken(Scanner.TOKEN_TYPE_RESERVED, "and")){ //Da -> Dr ( 'and' Dr )+ => 'and'
      Read_Next();
      procDR(); //This is to extra readToken() in procDR()
      treesToPop++;
    }
  ...
  private void procDR(){
    if(isCurrentToken(Scanner.TOKEN_TYPE_RESERVED, "rec")){ //Dr -> 'rec' Db => 'rec'
      Read_Next();
      procDB(); //This is to extra readToken() in procDB()
  ...
  private void procDB(){
    if(Is_Cur_Token_Type(Scanner.TOKEN_TYPE_L_PAREN)){ //Db -> '(' D ')'
      ...
      if(isCurrentToken(Scanner.TOKEN_TYPE_OPERATOR, ",")){ //Db -> Vl '=' E => '='
        Read_Next();
        procVL(); //This is to extra readNT in procVB()
        if(!isCurrentToken(Scanner.TOKEN_TYPE_OPERATOR, "="))
          throw new ParseException("DB: = expected.");
        ...
        if(isCurrentToken(Scanner.TOKEN_TYPE_OPERATOR, "=")){ //In here Db -> Vl '=' E => '='; if Vl had only one IDENTIFIER (no commas)
          Read_Next();
          Proc_E(); //this is to extra readNT in procE()
          Build_NAry_ASTNode(ASTNodeType.EQUAL, 2);
        }
        else{ //In here Db -> '<IDENTIFIER>' Vb+ '=' E => 'fcn_form'
          int treesToPop = 0;

          while(Is_Cur_Token_Type(Scanner.TOKEN_TYPE_IDENTIFIER) || Is_Cur_Token_Type(Scanner.TOKEN_TYPE_L_PAREN)){
            procVB(); //This is to extra readNT in procVB()
            treesToPop++;
          }
          ...
          Build_NAry_ASTNode(ASTNodeType.FCNFORM, treesToPop+2); //In here +1 for the last E and +1 for the first identifier
        }
      }
    }
  }
  ...
  private void procVL(){
    if(!Is_Cur_Token_Type(Scanner.TOKEN_TYPE_IDENTIFIER))
      ...
      Read_Next();
      int treesToPop = 0;
      while(isCurrentToken(Scanner.TOKEN_TYPE_OPERATOR, ",")){
        Read_Next();
        if(!Is_Cur_Token_Type(Scanner.TOKEN_TYPE_IDENTIFIER))
          throw new ParseException("VL: Identifier expected");
        Read_Next();
      ...
      if(treesToPop > 0) Build_NAry_ASTNode(ASTNodeType.COMMA, treesToPop+1);
    }
  }
